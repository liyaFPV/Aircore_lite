...
void build_plot_js() {
    GP.SEND(F("<script>\n"
              ...
              "var H=400,padL=50,padR=12,padT=28,padB=28;\n"
              "var lb=[],tip=null;\n"
              "function draw(){\n"
              ...
              "for(var i=0;i<N;i++)if(vis[i])for(var j=0;j<P[i].length;j++){var v=P[i][j];if(v<lo)lo=v;if(v>hi)hi=v;}\n"
              ...
              "x.stroke();\n"
              "}\n"
              "x.font='12px Arial';x.textAlign='left';x.textBaseline='top';var lx=padL+4;\n"
              "for(var i=0;i<N;i++){\n"
              "x.fillStyle=cols[i];x.fillRect(lx,padT+4,12,3);\n"
              "x.fillStyle=vis[i]?'#cfd8dc':'#5b636b';\n"
              "x.fillText(names[i],lx+16,padT-2);\n"
              "var ww=x.measureText(names[i]).width;\n"
              "lb[i]={x0:lx,x1:lx+ww+16+8};\n"
              "if(!vis[i]){x.strokeStyle='#5b636b';x.beginPath();x.moveTo(lx,padT+4);x.lineTo(lx+ww+16,padT+4);x.stroke();}\n"
              "lx+=ww+16+8+14;\n"
              "}\n"
              "if(tip){\n"
              "var t=tip.t;\n"
              "var px=padL+(t-tmin)/(tmax-tmin)*pw;\n"
              "x.strokeStyle='#4a5560';x.beginPath();x.moveTo(px,padT);x.lineTo(px,padT+ph);x.stroke();\n"
              "var rows=[],mw=0;\n"
              "x.font='12px Arial';\n"
              "for(var i=0;i<N;i++){\n"
              "if(!vis[i])continue;\n"
              "var idx=Math.round((now-t)/step);\n"
              "var j=P[i].length-1-idx;\n"
              "if(j<0||j>=P[i].length)continue;\n"
              "var v=P[i][j];\n"
              "x.fillStyle=cols[i];x.beginPath();x.arc(px,padT+ph-(v-lo)/span*ph,3,0,Math.PI*2);x.fill();\n"
              "var tw=x.measureText(names[i]+': '+v.toFixed(1)).width;\n"
              "if(tw>mw)mw=tw;\n"
              "rows.push([i,v]);\n"
              "}\n"
              "if(rows.length){\n"
              "var bx=px+10;if(bx+mw+20>W)bx=px-mw-30;\n"
              "var by=padT+4;\n"
              "x.fillStyle='rgba(17,22,29,0.92)';x.fillRect(bx,by,mw+20,rows.length*18+8);\n"
              "x.textAlign='left';x.textBaseline='middle';\n"
              "for(var k=0;k<rows.length;k++){\n"
              "x.fillStyle=cols[rows[k][0]];x.fillRect(bx+6,by+8+k*18,10,3);\n"
              "x.fillStyle='#cfd8dc';x.fillText(names[rows[k][0]]+': '+rows[k][1].toFixed(1),bx+20,by+10+k*18);\n"
              "}\n"
              "}\n"
              "}\n"
              "}\n"
              ...
              "for(var i=0;i<N;i++)if(mx>=lb[i].x0-4&&mx<=lb[i].x1+4){vis[i]=!vis[i];tip=null;draw();return;}\n"
              "});\n"
              "cv.addEventListener('mousemove',function(e){\n"
              "var r=cv.getBoundingClientRect();\n"
              "var mx=e.clientX-r.left,my=e.clientY-r.top;\n"
              "if(mx<padL||mx>cv.width-padR||my<padT||my>cv.height-padB){tip=null;draw();return;}\n"
              "var now=Date.now(),tmin=now-(max-1)*step,tmax=now;\n"
              "tip={t:tmin+(tmax-tmin)*(mx-padL)/(cv.width-padL-padR)};draw();\n"
              "});\n"
              "cv.addEventListener('mouseleave',function(){tip=null;draw();});\n"
